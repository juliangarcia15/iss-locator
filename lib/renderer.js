...
MainView.prototype.notifyPassTime = function (form, self) {
    ...
    if (!data[0].value) { // NOTE find better validation library?
        $(form).find('input[name=address]').addClass('not-valid');
        return false;
    }
...
MainView.prototype.bindEvents = function () {
    ...
    $nextPassesForm.find('input[name=address]').on('focus', function(){
